// Napeče "rozpadovou kostku" pro onkologickou incidenci z NOR (NR-07-01).
// Rozměry: rok × diagnóza × věková skupina (5letá) × pohlaví × stadium.
// Jen kurátorované primární diagnózy (tím vypadnou artefaktové kódy: metastázy C78/C79,
// neurčité C76/C80 apod.). Nehodgkinské lymfomy sloučeny (C82–C86) kvůli překlasifikaci.
// Výstup: data/cubes/onkologie_incidence.json (řídké buňky + metadata dimenzí + sken anomálií).
//
...
const SRC = 'https://data.mzcr.cz/data/distribuce/372/Otevrena-data-NR-07-01-incidence-prevalence-zhoubne-nadory-regiony-cr-2024-01.csv';
const SOURCE_URL = 'https://www.nzip.cz/data/2054-incidence-prevalence-zhoubne-nadory';
const YEAR_FROM = 2000; // omezíme na novější roky (relevance + velikost)

// Kurátorované primární diagnózy: kód MKN (3 znaky) → {key, name}. Plné názvy, bez zkratek.
// Co tu není, do kostky nejde (vypadne dlouhý ocas vzácných a artefaktových kódů).
...
function ageBandStart(code) {
  // vek_kategorie_kod_dg např. "66045049" → dolní mez 045 = 45 → pásmo 45–49.
  const low = parseInt((code || '').toString().slice(2, 5), 10);
  if (!Number.isFinite(low)) return null;
  return Math.floor(low / 5) * 5; // jistota zarovnání na 5letá pásma
}
...
// Akumulace: composite klíč → count. Plus marginály pro anomálie.
...
await new Promise((res, rej) => Papa.parse(ns, {
  ...
  step: ({ data: x }) => {
    nRows++;
    const dg3 = (x.diagnoza_kod || '').toString().slice(0, 3);
    const d = DG[dg3]; if (!d) return;
    const y = parseInt((x.rok_dg || '').toString(), 10); if (!Number.isFinite(y) || y < YEAR_FROM) return;
    const a = ageBandStart(x.vek_kategorie_kod_dg); if (a == null) return;
    const sex = SEX((x.pohlavi || '').toString()); if (!sex) return;
    const st = STAGE(x.stadium);
    const key = `${y}|${d.key}|${a}|${sex}|${st}`;
    counts.set(key, (counts.get(key) || 0) + 1);
    dgTotal[d.key] = (dgTotal[d.key] || 0) + 1;
    nUsed++;
  },
  complete: res, error: rej,
}));

// Sestav dimenze (stabilní pořadí). Diagnózy podle celkového objemu sestupně.
const dgNameByKey = {};
...
const stageDim = ['I', 'II', 'III', 'IV', 'neuvedeno'];
...
const si = Object.fromEntries(sexDim.map((s, i) => [s, i]));
const sti = Object.fromEntries(stageDim.map((s, i) => [s, i]));

const cells = [];
for (const [k, n] of counts) {
  const [y, dk, a, sex, st] = k.split('|');
  cells.push([yi[+y], di[dk], ai[+a], si[sex], sti[st], n]);
}

// ---- Sken anomálií (z marginálů) ----
...
const maxJump = (o) => { // největší meziroční relativní skok → indikátor možné změny kódování
  let m = 0; for (let i = 1; i < years.length; i++) { const a = o[years[i - 1]] || 0, b = o[years[i]] || 0; if (a >= 30) m = Math.max(m, Math.abs((b - a) / a)); }
  return m;
};
const anomalies = [];
for (const d of diagnosis) {
  const k = d.key, sName = d.name;
  const b = avg(byDgYear[k], BASE), rc = avg(byDgYear[k], REC);
  if (rc >= 150 && b > 0) {
    const pct = Math.round((rc - b) / b * 100);
    const artifact = maxJump(byDgYear[k]) > 0.35;
    if (Math.abs(pct) >= 15) anomalies.push({ type: 'trend', dg: k, name: sName, pct, from: Math.round(b), to: Math.round(rc), artifact_risk: artifact });
  }
  const yo = byDgPerYoung[k];
  if (yo && yo.r.t >= 800) {
    const sb = yo.b.t ? yo.b.y / yo.b.t * 100 : 0, sr = yo.r.t ? yo.r.y / yo.r.t * 100 : 0;
    if (Math.abs(sr - sb) >= 2.5) anomalies.push({ type: 'vek_posun', dg: k, name: sName, from_pct: +sb.toFixed(1), to_pct: +sr.toFixed(1), diff: +(sr - sb).toFixed(1) });
  }
  const la = byDgPerLate[k];
  if (la && la.r.staged >= 400) {
    const lb = la.b.staged ? la.b.late / la.b.staged * 100 : 0, lr = la.r.staged ? la.r.late / la.r.staged * 100 : 0;
    if (Math.abs(lr - lb) >= 3) anomalies.push({ type: 'stadium_posun', dg: k, name: sName, from_pct: +lb.toFixed(1), to_pct: +lr.toFixed(1), diff: +(lr - lb).toFixed(1) });
  }
}
// Střídáme typy (trend / posun věku / posun stadia), ať se v popředí neztratí ty cennější
// úhly (mladí, pozdní záchyt) jen proto, že % trendů má větší absolutní čísla než body.
const byType = (t) => anomalies.filter(a => a.type === t).sort((a, b) => Math.abs(b.diff ?? b.pct) - Math.abs(a.diff ?? a.pct));
const tr = byType('trend'), ve = byType('vek_posun'), st = byType('stadium_posun');
const mixed = [];
...
anomalies.length = 0; anomalies.push(...mixed);
...
const cube = {
  ...
  source: 'Národní onkologický registr (ÚZIS ČR)',
  ...
  dims: { diagnosis, age, sex: sexDim, stage: stageDim, years },
  cells,
  anomalies,
};
mkdirSync('data/cubes', { recursive: true });
writeFileSync('data/cubes/onkologie_incidence.json', JSON.stringify(cube) + '\n');

const secs = ((Date.now() - t0) / 1000).toFixed(0);
console.log(`HOTOVO za ${secs}s | řádků ${nRows.toLocaleString()} → použito ${nUsed.toLocaleString()}`);
console.log(`dimenze: ${diagnosis.length} diagnóz, ${age.length} věk. pásem, ${years.length} let (${years[0]}–${years[years.length - 1]}), 2 pohlaví, 5 stadií`);
console.log(`buněk: ${cells.length.toLocaleString()} | velikost JSON: ${(JSON.stringify(cube).length / 1024).toFixed(0)} KB`);
console.log(`\nanomálie (${anomalies.length}):`);
for (const a of anomalies.slice(0, 18)) {
  if (a.type === 'trend') console.log(`  [trend]   ${(a.pct > 0 ? '+' : '') + a.pct}%  ${a.name} (${a.from}→${a.to}/rok)${a.artifact_risk ? '  ⚠ možná změna kódování' : ''}`);
  if (a.type === 'vek_posun') console.log(`  [věk]     ${(a.diff > 0 ? '+' : '') + a.diff} b.b. mladších 50  ${a.name} (${a.from_pct}%→${a.to_pct}%)`);
  if (a.type === 'stadium_posun') console.log(`  [stadium] ${(a.diff > 0 ? '+' : '') + a.diff} b.b. pozdní záchyt  ${a.name} (${a.from_pct}%→${a.to_pct}%)`);
}
